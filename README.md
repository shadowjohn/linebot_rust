...
LINE Desktop 自動化發送流程示意：

![linebot_rust 操作畫面](snapshot/01.png)

示範影片：[`snapshot/01.mp4`](snapshot/01.mp4)
...
* 新增 LINE 主視窗右側面板模式支援：優先在主視窗右側聊天面板直接發送，若偵測不到輸入框才 fallback 開啟獨立聊天室視窗。
* 支援文字與圖片合併發送：同一筆派工同時含文字與檔案時，會開啟聊天室一次並合併送出。
* 優化聊天室開啟流程：先使用既有 LINE 主視窗，避免每次呼叫 `LineLauncher.exe`。
* 搜尋結果與聊天室視窗改為輪詢等待，降低固定延遲。
* 新增 Unicode 剪貼簿讀回驗證，避免中文被轉成 `???` 後仍送出。
* 新增本機錯誤日誌：`log\YYYYMMDD.txt`。
